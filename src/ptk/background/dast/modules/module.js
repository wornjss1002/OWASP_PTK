...
export class ptk_module {
    constructor(module) {
        ...
        this.nonAttackParams = ['csrf', '_csrf', /^x-.*-token$/i]
    ...
    // Build N atomic attacks (default) or 1 bulk attack if action.atomic === false
    buildAttacks(schema, attack) {
        const prepared = this.prepareAttack(attack)
        const atomic = prepared.atomic !== false   // <-- keep exactly as you asked
        const attacks = []
        ...
        for (const tgt of targets) {
            ...
            _schema.metadata.attacked = mutations.length ? mutations[0] : { location: tgt.location, name: tgt.name }
    ...
    buildAttack(schema, attack) {
        ...
        // else leave undefined to avoid "random cookie" selection.
